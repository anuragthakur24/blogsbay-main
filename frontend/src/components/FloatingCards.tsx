  ...
  return (
    ...
    >
      ...
      <div style={{ transform: `rotate(${rotation}deg)` }}>
        <motion.div
          className="w-28 h-30 lg:w-40 lg:h-44 mx-1 my-7 rounded-xl backdrop-blur-lg shadow-2xl bg-gradient-to-br border border-white/10 from-[#334264]/40 to-[#111c34] flex flex-col justify-center items-center text-white relative"
          animate={{ rotateY: flipped ? 180 : 0 }}
          transition={{ duration: 0.5 }}
          style={{ transformStyle: "preserve-3d" }}
        >
          {!flipped ? (
            <div className="flex flex-col p-5 w-28 h-30 lg:w-36 lg:h-40 rounded-lg items-center justify-center text-center border border-white/20">
              <h3 className="lg:text-lg font-semibold text-sm">{title}</h3>
              <p className="lg:text-sm text-gray-200 mt-2 text-xs">{content}</p>
            </div>
          ) : (
            <div
              className="flex flex-col p-8 w-28 h-30 lg:w-36 lg:h-40 rounded-lg items-center justify-center text-center"
